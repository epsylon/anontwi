...
AnonTwiIRCbot commands: 
    ...
    !public [user]    : view public timeline of a user
...
-----------------------
"""
HELLO = """Hello, I am an AnonTwi-IRCbot (http://anontwi.sf.net). Use: !help for options."""
class AnonTwiIrcBot():
...
#            self.s.send ( 'PRIVMSG ' + who + " :" + "------------------------------" + "\n")
        ...
        if (a=="!home") or (a=="!home\r"):
            ...
            try:
                num_ocurrences = int(string.join(args[1:]," ")) 
            except:
                num_ocurrences = 2        
            h = HTMLParser.HTMLParser()
            try:
                if num_ocurrences is None or num_ocurrences <=0:
                    num_ocurrences = 2
                (timelines, num) = self.wrapper.home_timeline(num_ocurrences)
                self.s.send ( 'PRIVMSG ' + who + " :" + "===================" + "\n")
        ...
        if (a=="!public") or (a=="!public\r"):
            ...
            h = HTMLParser.HTMLParser()
            try:
                user = string.join(args[1:]," ") + "\n"
                num_ocurrences = "4" # fix this to provide a num of results
                ps = self.wrapper.show_public(user, num_ocurrences)
                self.s.send ( 'PRIVMSG ' + who + " :" + "===================" + "\n")
        ...
        if (a=="!private") or (a=="!private\r"):
            ...
            try:
                num_ocurrences = string.join(args[1:]," ")
            except:
                num_ocurrences = 2     
            if num_ocurrences is None or num_ocurrences <= "0":
                num_ocurrences = "2"  
            h = HTMLParser.HTMLParser()
            try:
                (privates, num) = self.wrapper.show_private(num_ocurrences)
                self.s.send ( 'PRIVMSG ' + who + " :" + "===================" + "\n")
