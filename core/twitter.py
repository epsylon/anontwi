...
class User(object):
  ...
  location = property(GetLocation, SetLocation,
                      doc='The geographic location of this status message.')
...
class Api(object):
  ...
  Example usage:
    ...
    There are many other methods, including:
      ...
      >>> api.CreateBlock(user)
      >>> api.DestroyBlock(user)
  '''
  ...
  def __init__(self,
    ...
    if base_url is None:
      if self.source_api == "identi.ca/api":
          self.base_url = 'http://'+self.source_api+''
      else:
          self.base_url = 'https://'+self.source_api+'/1'
    else:
  ...
  def PostUpdates(self, status, continuation=None, reply=None, latitude=None, longitude=None):
    ...
    if continuation is None:
      continuation = ''
    pattern = '@[a-zA-Z0-9_]+'
    user = re.search(pattern, status)

    if user is None:
        line_length = CHARACTER_LIMIT - len(continuation)
    else:
        line_length = CHARACTER_LIMIT - len(continuation) - len(user.group(0)) - 1
        status = status[len(user.group(0))+1:len(status)]
    
    lines = textwrap.wrap(status, line_length)
    
    for line in lines[0:-1]:
      if user is not None:
          line = unicode(user.group(0)) + " " +  line
      results.append(self.PostUpdate(status=line + continuation, in_reply_to_status_id=reply, latitude=latitude, longitude=longitude))
    
    if user is None:
        line = lines[-1]
    else:
        line = unicode(user.group(0)) + " " +  lines[-1]
    results.append(self.PostUpdate(status=line, in_reply_to_status_id=reply, latitude=latitude, longitude=longitude))
  ...
  def PostRetweet(self, id):
    ...
    try:
        ...
        raise TwitterError("'id' must be an integer")
    url = 'http://'+self.source_api+'/1/statuses/retweet/%s.json' % id
  ...
  def CreateBlock(self, 
    ...
    Args:
      screen_name:
         ...
         Helpful for disambiguating when a valid screen name 
         is also a user ID. [optional]
      user_id:
         The ID of the potentially blocked user. 
         Helpful for disambiguating when a valid user ID 
         is also a valid screen name. [optional]
      include_entities:
         When set to either true, t or 1, each tweet will 
         include a node called "entities". [optional]
      skip_status:
         When set to either true, t or 1 statuses 
         will not be included in the returned user objects. [optional]  
    '''
    if not self._oauth_consumer:
      raise TwitterError("The twitter.Api instsance must be authenticated.")

    url  = '%s/blocks/create.json' % self.base_url
    json = self._FetchUrl(url, post_data={'screen_name': screen_name})
    data = self._ParseAndCheckTwitter(json)
  ...
  def CreateFavorite(self, status):
    ...
    '''
    url  = '%s/favorites/create/%s.json' % (self.base_url, status)
    json = self._FetchUrl(url, post_data={'id': status})
    data = self._ParseAndCheckTwitter(json)
    return Status.NewFromJsonDict(data)
  ...
  def DestroyFavorite(self, status):
    ...
    '''
    url  = '%s/favorites/destroy/%s.json' % (self.base_url, status)
    json = self._FetchUrl(url, post_data={'id': status})
    data = self._ParseAndCheckTwitter(json)
    return Status.NewFromJsonDict(data)
...
#    return self._location
  ...
  def SetLocation(self, location=None): #changed
    '''Set the geolocation associated with this status message
  ...
  def _InitializeUserAgent(self):
    ...
    agents = []
    agents.append('Mozilla/5.0 (iPhone; U; CPU iOS 2_0 like Mac OS X; en-us)')
    agents.append('Mozilla/5.0 (Linux; U; Android 0.5; en-us)')
    agents.append('Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.0)')
    agents.append('Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)')
    agents.append('Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.13 (KHTML, like Gecko) Chrome/0.2.149.29 Safari/525.13')
    agents.append('Opera/9.25 (Windows NT 6.0; U; en)')
    agents.append('Mozilla/2.02E (Win95; U)')
    agents.append('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)')
    agents.append('Mozilla/5.0 (compatible; Yahoo! Slurp; http://help.yahoo.com/help/us/ysearch/slurp)')
    user_agent = random.choice(agents).strip() 
  ...
  def _FetchUrl(self,
    ...
    # Add fake User-Agent and Referer
    opener.addheaders = [('User-agent', self._request_headers['User-Agent']), ('Referer', '')]
...
class VerifiedHTTPSConnection(httplib.HTTPSConnection):
  """Class to verify SSL Twitter Certs"""
  """See also: http://docs.python.org/library/ssl.html"""
  def __call__():
      a = None    
  ...
  def connect(self):
      ...
      sock = socket.create_connection((self.source_api, 443))
      ...
      cert = os.path.join( os.path.dirname(os.path.abspath(__file__)) + "/certs/", "cacert.pem")
...
class VerifiedHTTPSHandler(urllib2.HTTPSHandler):
  """Class to verify SSL Twitter handler"""
